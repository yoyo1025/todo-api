...
func (m *mockTaskQueryRepository) FindAllTask(userId int64) ([]*response.Task, error) {
	args := m.Called(userId)
	tasks, _ := args.Get(0).([]*response.Task)
	return tasks, args.Error(1)
}

func (m *mockTaskQueryRepository) FindTaskById(taskId int64) (*response.Task, error) {
	// 今回のインタフェーステストでは未使用
	return nil, nil
}
...
func TestFindAllTask_Failure(t *testing.T) {
	mockRepo := new(mockTaskQueryRepository)
	mockRepo.On("FindAllTask", int64(2)).Return(nil, errors.New("db error"))
